...
module Solution = {
  ...
  let updateEqualNumber = (numberList, number1, number2) => {
    number1 == number2 ? [number1, ...numberList] : numberList;
  };
  ...
  let make = captcha => {
    captcha
    ->makeMatchesNumbers(~fullCaptcha=captcha, ~result=[])
    ->Belt.List.reduce(0, (+));
  };
};
...
module Solution2 = {
  let updateEqualNumber = (numberList, number1, number2) => {
    number1 == number2 ? [number1 * 2, ...numberList] : numberList;
  };

  let rec makeMatchesNumber = (door: Door.t, ~result) => {
    switch (
      door.h->Belt.List.head,
      door.t->Belt.List.head,
      door.h->Belt.List.tail,
    ) {
    | (Some(_), Some(_), None)
    | (Some(_), None, _)
  ...
  let make = door => {
    door->makeMatchesNumber(~result=[])->Belt.List.reduce(0, (+));
  };
};
