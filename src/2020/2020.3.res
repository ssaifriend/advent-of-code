...
module Grounds = {
  ...
  let make = inputStrs =>
    inputStrs
    ->Js.String2.split("\n")
    ->Belt.Array.map(row => Js.String.split("", row)->Belt.Array.map(Ground.make))
}
...
module TreeCounter = {
  ...
  let walk = (grounds, pattern) => {
    ...
    let walked = Belt.Array.rangeBy(
      0,
      groundRowLength - 1,
      ~step=pattern.y,
    )->Belt.Array.reduce(initial, (walk, row) => {
      coord: {
        x: mod(walk.coord.x + pattern.x, groundColLength),
        y: row,
      },
      list: list{grounds[row][walk.coord.x], ...walk.list},
    })

    walked.list->Belt.List.keep(ground => ground.hasTree)->Belt.List.size->Belt.Float.fromInt
  }
}
...
let eachTreeCount = movePatterns->Belt.Array.map(pattern => grounds->TreeCounter.walk(pattern))
