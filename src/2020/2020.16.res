module Field = {
  ...
  let make = inputStr => {
    let re = Js.Re.fromString("([a-z ]+): ([0-9]+)-([0-9]+) or ([0-9]+)-([0-9]+)")
  ...
  let isNumberInRange = (number, start, end_) => number >= start && number <= end_
...
module InvalidTicketScanner = {
  let rec isValidTicket = (ticket, fields) => {
    ...
    result || fields->Belt.Array.size == 1
      ? result
      : ticket->isValidTicket(fields->Belt.Array.sliceToEnd(1))
  }
...
let fields = inputStrs[0]->Js.String2.split("\n")->Belt.Array.map(Field.make)

let myTickets = inputStrs[1]->Js.String2.split("\n")->Belt.Array.sliceToEnd(1)->Tickets.make
...
module TicketsGroup = {
  ...
  let make = inputStrs => {
    ...
    Belt.Array.range(0, groupSize - 1)->Belt.Array.map(index =>
      rows->Belt.Array.map(row => row[index])
    )
  }
}
...
module PositionFinder = {
  ...
  let update = (ablePositions, positions) => {
    let updatedPositions =
      ablePositions->Belt.Array.reduceWithIndex(positions, (positions, ablePosition, index) =>
        ablePosition->Belt.Array.size == 1
          ? positions->Belt.Map.String.set(ablePosition[0], index)
          : positions
      )
    ...
    {remainPositions: remainPositions, mappedPositions: updatedPositions}
  }
  ...
  let find = (fields, ticketsGroup) => {
    let ablePositions =
      ticketsGroup->Belt.Array.map(ticketGroup => ticketGroup->ablePositions(fields))
    let initial = {
  ...
  let mappedPositionsToMyTicketMap = (mappedPositions, myTicket) => {
    ...
    mappedPositions->Belt.Map.String.reduce(initialMyTickets, (
      mappedMyTicket,
      mappedPosition,
      index,
...
let myTickets2 = inputStrs2[1]->Js.String2.split("\n")->Belt.Array.sliceToEnd(1)->Tickets.make

let nearByTickets2 =
  inputStrs2[2]->Js.String2.split("\n")->Belt.Array.sliceToEnd(1)->TicketsGroup.make
