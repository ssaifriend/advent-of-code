...
let rec visit = (directions, (x, y), visits) => {
  let next =
    switch (directions->Belt.List.head) {
  ...
  | (None, _)
  | (Some(_), None) => visits
  | (Some(coord), Some(tail)) => tail->visit(coord, [coord, ...visits])
  };
};
...
let unique = visits =>
  visits
  ->Belt.List.toArray
  ->Belt.Set.fromArray(~id=(module CoordComp))
  ->Belt.Set.toArray;

[">", "^>v<", "^v^v^v^v^v"]
->Belt.List.map(parse)
->Belt.List.map(directions => directions->visit((0, 0), [(0, 0)]))
...
let splitRobo = directions => {
  Belt.Array.range(0, 1)
  ->Belt.Array.map(index => {
      directions->Belt.List.keepWithIndex((_, dIndex) => dIndex mod 2 == index)
    });
};

let visitsEachSanta = (directionsWithRobo) => {
  directionsWithRobo->Belt.Array.map(directions =>
    directions->visit((0, 0), [(0, 0)])->Belt.List.toArray
  )
