...
let hasLeastOneLetterTwice = str => {
  ...
  Belt.Array.zip(str1, str2)
  ->Belt.Array.keep(((str1, str2)) => str1 == str2)
  ->Belt.Array.size
  > 0;
};
...
let isNice = str =>
  str->hasLeastThreeVowels
  && str->hasLeastOneLetterTwice
  && str->hasNotContainString;

"ugknbfddgicrmopn"->isNice->Js.log; //T
"aaa"->isNice->Js.log; //T
"jchzalrnumimnmhp"->isNice->Js.log; //F
"haegwjzuvuyypxyu"->isNice->Js.log; //F
"dvszwmarrgswjxmb"->isNice->Js.log; //F
...
let hasLeastTwoLetterTwice = str => {
  ...
  ->Belt.Array.keepWithIndex((findStr, index) => {
      str
      ->Js.String2.sliceToEnd(~from=index + 2)
      ->Js.String2.includes(findStr)
    })
  ->Belt.Array.size
  > 0;
};
...
let isBetterNice = str =>
  str->hasLeastTwoLetterTwice
  && str->Js.String2.split("")->hasOneLetterRepeat(~index=0);

"qjhvhtzxzqqjkmpb"->isBetterNice->Js.log; //T
"xxyxx"->isBetterNice->Js.log; //T
"uurcxstgmygtbstg"->isBetterNice->Js.log; //F
"ieodomkazucvgmuy"->isBetterNice->Js.log; //F
