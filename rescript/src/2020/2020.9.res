...
let rec makeNumberIndexSets = (indexes, sets, ~setCount) =>
  if sets->Belt.List.size == 0 {
    ...
    setCount == 1 ? newSets : indexes->makeNumberIndexSets(newSets, ~setCount=setCount - 1)
  }
...
let rec findNotMatchingNumber = (numbers, ~preamble) => {
  ...
  let twoNumberIndexSets =
    ...
    ->Belt.List.fromArray
    ->makeNumberIndexSets(list{}, ~setCount=2)
...
let rec findContinueNumber = (numbers, ~findNumber, ~maxContinueNumberCount) =>
  ...
  | Some(a) => Some(a)
  | None =>
    numbers->findContinueNumber(~findNumber, ~maxContinueNumberCount=maxContinueNumberCount + 1)
  }

let result = switch numbers->findContinueNumber(
  ~findNumber=findedNumber,
  ~maxContinueNumberCount=2,
) {
| Some(numbers) =>
  let numberSet = numbers->Belt.Array.map(Belt.Float.toInt)->Belt.Set.Int.fromArray
