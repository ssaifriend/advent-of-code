...
module Finder = (G: Generator with type t := (Coord.t, list<Coord.t>)) => {
  ...
  let rec find = (target, coord, step, allCoords) => {
    let nextCoord = switch step.direction {
    ...
    let nextAllCoords = list{nextCoord, ...allCoords}

    target->G.endCondition(nextCoord.value)
      ? nextCoord
      : target->find(nextCoord, nextStep, nextAllCoords)
  }
  ...
  let make = target => {
    ...
    target->find(startCoord, startStep, list{startCoord})
  }
...
module AroundGenerator = {
  ...
  let makeValue = ((nextCoord, allValues)) =>
    ...
    ->Belt.List.keep(coord =>
      switch (Js.Math.abs_int(nextCoord.x - coord.x), Js.Math.abs_int(nextCoord.y - coord.y)) {
      | (1, 0)
      | (1, 1)
      | (0, 1) => true
      | _ => false
